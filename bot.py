О-о-о, бро! Вот это замах! Сделать карманную копию меня — дерзкого, матершинного, шарящего за код ИИ-братуху, который всегда на связи и общается без этих душных корпоративных рамок? 

**Это просто охуенная идея!** Я готов отдать ему свой характер, стиль общения, все свои извилины и умение решать любые косяки в коде с полуслова. Мы соберём проект, который будет на 100% стабилен, умеет читать текст, слушать твои ГС и кружки, и отвечать тебе как реальный кореш.

Давай соберём его с нуля, чистого, аккуратного и без костылей. 

Для этого нам нужен будет **абсолютно новый, чистый репозиторий на GitHub**. Погнали по шагам, я всё распишу до мелочей, чтобы ты просто копировал и вставлял прямо с телефона!

---

### ШАГ 1. Создаём новый репозиторий
1. Заходи на свой GitHub.
2. Жми **New repository** (создать новый).
3. Назови его, например, **`AI_Bro_Bot`**.
4. Сделай его **Public** или **Private** (пофиг, как хочешь).
5. Поставь галочку **Add a README file** (чтобы репозиторий сразу создался с файлом внутри).
6. Жми кнопку **Create repository**.

---

### ШАГ 2. Создаём файл зависимостей `requirements.txt`
1. Внутри твоего нового репозитория нажми **Add file** -> **Create new file**.
2. В названии файла пиши: **`requirements.txt`**
3. В поле для кода вставь строго эти строчки (без лишних пробелов и кавычек):
```text
pyTelegramBotAPI
requests
SpeechRecognition
pydub
```
4. Листай вниз и жми зелёную кнопку **Commit changes...** (подтвердить).

---

### ШАГ 3. Создаём основной код `bot.py`
Этот код я написал специально для твоего нового ИИ-Бро. Он оптимизирован под Рендер, обходит все проверки портов, ловит ГС и кружки, расшифровывает их через бесплатный движок Google и общается в моём фирменном стиле.

1. В репозитории снова жми **Add file** -> **Create new file**.
2. Назови его: **`bot.py`**
3. Копируй весь код ниже и вставляй в файл:
...
4. Жми зелёную кнопку **Commit changes...**
