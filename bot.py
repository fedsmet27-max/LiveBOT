import os, requests, base64, json, gc, time, asyncio
...
def process_media(file_id, chat_id, is_video=False):
    ...
    text, b64_img = "", None
    try:
        # Для простоты скачивание оставим синхронным, на скорость не повлияет
        file_info = bot.get_file(file_id)
        # Так как bot теперь асинхронный, некоторые методы требуют await, но get_file в telebot работает через сессию.
        # Для стабильности на телефоне обернем это в асинхронный вызов ниже в обработчиках.
        pass
    except Exception as e: print(f"Медиа ошибка: {e}")
    return text, b64_img

# Переписываем под асинхронность скачивание медиа
...
if __name__ == "__main__":
    # Запуск всего приложения в одном главном асинхронном событии
    asyncio.run(main())
